...
public class AttenDanModel extends BaseMvpModel {
    ...
    public void getListData(String string, final ResultCallback<String> resultCallback) {
        int length = string.length();
        MediaType mediaType = MediaType.parse("text/x-markdown; charset=utf-8");
        RequestBody requestBody = RequestBody.create(mediaType, string);
    ...
    public void insertData(String json, final ResultCallback<String> callback) {
    ...
    public void getTime(String json, final ResultCallback<String> callback) {
        int length = json.length();
        MediaType mediaType = MediaType.parse("text/x-markdown; charset=utf-8");
        RequestBody requestBody = RequestBody.create(mediaType, json);
        ...
        ApiService apiserver = HttpUtils.getInstance().getApiserver(
                ApiService.serverAddr, ApiService.class);
        apiserver.getTime(length, requestBody)
                .compose(RxUtils.<String>rxObserableSchedulerHelper())
                .subscribe(new BaseObserver<String>() {
                    @Override
                    public void onNext(String s) {
